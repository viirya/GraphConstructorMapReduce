...
public class GraphConstructorMapReduceMinHashPreComputeProgressive {
...
*/
    ...
    public static class AddMinHashInfoMapper extends Mapper<LongWritable, Text, Text, Text> {
        public void map(LongWritable key, Text value, Context context) throws NumberFormatException, IOException, InterruptedException {
            ...
            if (test_tokenizer.countTokens() > 1) {
                ...
                test_tokenizer.nextToken();                String cluster_id = test_tokenizer.nextToken();
                ...
                if (image_ids.length > min_cluster_size) {
                    for (int i = 0; i < image_ids.length; i++) {
                        for(int j = i + 1; j < image_ids.length; j++) {
                            StringTokenizer image_id_i_tokenizer = tokenize(image_ids[i], "#%");
                            StringTokenizer image_id_j_tokenizer = tokenize(image_ids[j], "#%");

                            String image_id_i = image_id_i_tokenizer.nextToken();
                            String image_id_j = image_id_j_tokenizer.nextToken();
                            String image_id_i_sampled_ret = image_id_i_tokenizer.nextToken();
                            String image_id_j_sampled_ret = image_id_j_tokenizer.nextToken();

                            if ((!image_id_i_sampled_ret.equals(image_id_j_sampled_ret)) ||
                                (image_id_i_sampled_ret.equals("in") && image_id_j_sampled_ret.equals("in"))) {
                                if(image_ids[i].compareTo(image_ids[j]) < 0) {
                                    image_pair.set("(" + image_id_i + "," + image_id_j + ")");
                                } else {
                                    image_pair.set("(" + image_id_j + "," + image_id_i + ")");
                                }
                            }
    ...
    public static class InvertedListConstructorReducer extends Reducer<Text, Text, Text, Text> {
        public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
            ...
            StringTokenizer image_feature_tokenizer = tokenize(strbuf.toString(), " ()[]#%");
            
            int number_of_images = image_feature_tokenizer.countTokens() / 3;
            String[] image_id = new String[number_of_images];
            String[] sampled_result = new String[number_of_images];
            ...
            while (image_feature_tokenizer.hasMoreTokens()) {
                ...
                sampled_result[count++] = image_feature_tokenizer.nextToken().toString();
            }
            ...
            for(int i = 0; i < image_id.length; i++) {
                for(int j = i + 1; j < image_id.length; j++) {
                    ...
                    if ((!sampled_result[i].equals(sampled_result[j])) ||
                        (sampled_result[i].equals("in") && sampled_result[j].equals("in"))) {
                        ...
                        if(feature_tf_idf_value[i] > threshold && feature_tf_idf_value[j] > threshold) {
                            ...
                            //similarity.set(new Double(feature_tf_idf_normalized_value[i]).toString() + "," + new Double(feature_tf_idf_normalized_value[j]).toString());        
                            context.write(image_pair, new Text(similarity.toString()));
                        }
                    }
                }
    ...
    public static class PairwiseSimilarityComputationMapper extends Mapper<LongWritable, Text, Text, DoubleWritable> {
        public void map(LongWritable key, Text value, Context context) throws NumberFormatException, IOException, InterruptedException {
            ...
            String image_pair = feature_id_tokenizer.nextToken();
            String image_id_and_feature_value = feature_id_tokenizer.nextToken();

            if (image_id_and_feature_value.equals("-1")) {
                context.write(new Text(image_pair), new DoubleWritable(-1.0d));
            } else {
                context.write(new Text(image_pair), new DoubleWritable(Double.parseDouble(image_id_and_feature_value)));     
            }
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (args.length < 3) {
            System.out.println("Usage: GraphConstructorMapReduceMinHashPreComputeProgressive <input path> <cluster path> <threshold> [compress]");    
            System.exit(0);
        }
        ...
        addminhashinfo(args[1], 0);
        construct_inverted_list("output/graph_data/idf", Double.parseDouble(args[2]));
    ...
    public static Pair calculate_adl(String input_path) throws Exception {
        ...
        job_adl_calculator.setJarByClass(GraphConstructorMapReduceMinHashPreComputeProgressive.class);
    ...
    public static void calculate_tf(String input_path, double adl) throws Exception {
        ...
        job_tfidf_tf_calculator.setJarByClass(GraphConstructorMapReduceMinHashPreComputeProgressive.class);
    ...
    public static void calculate_idf(String input_path, int number_of_images) throws Exception {
        ...
        job_tfidf_idf_calculator.setJarByClass(GraphConstructorMapReduceMinHashPreComputeProgressive.class);
    ...
    public static void addminhashinfo(String cluster_path, int min_cluster_size) throws Exception {
        ...
        conf.setLong("dfs.block.size",134217728);
        conf.set("mapred.child.java.opts", "-Xmx2048m");
        conf.set("MinClusterSize", new Integer(min_cluster_size).toString());        if (compression)
            setJobConfCompressed(conf);
        Job job_addminhashinfo = new Job(conf);
        job_addminhashinfo.setJarByClass(GraphConstructorMapReduceMinHashPreComputeProgressive.class);
    ...
    public static void construct_inverted_list(String input_path, double threshold) throws Exception {
        ...
        job_inverted_list_constructor.setJarByClass(GraphConstructorMapReduceMinHashPreComputeProgressive.class);
    ...
    public static void compute_pairwise_similarity(String input_path, double threshold) throws Exception {
        ...
        job_pairwise_similarity_computation.setJarByClass(GraphConstructorMapReduceMinHashPreComputeProgressive.class);
        job_pairwise_similarity_computation.setJobName("Computing Pairwise Similarity");

        FileInputFormat.setInputPaths(job_pairwise_similarity_computation, new Path(input_path + "/*"), new Path("output/graph_data/image_pairs/*"));
