...
public class GraphConstructorMapReduceMinHashFeature {
...
*/
            ...
            for (int i = 0; i < feature_id.length; i++) {
                if(feature_value[i] > 0) {
                    ...
                    for (int j = 0; j < cluster_ids.length; j++) {
                        key_for_image_feature_id.set(new Integer(feature_id[i]).toString() + "," + cluster_ids[j]);
            ...
            for(int i = 0; i < image_id.length; i++) {
                for(int j = i + 1; j < image_id.length; j++) {
                    if(feature_tf_idf_value[i] > threshold && feature_tf_idf_value[j] > threshold) {
                        ...
                        similarity.set(new Double(feature_tf_idf_normalized_value[i] * feature_tf_idf_normalized_value[j]).toString() + "," + feature_id_and_cluster_id[0]);        
    ...
    public static class PairwiseSimilarityComputationReducer extends Reducer<Text, Text, Text, Text> {
        public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
            ...
            HashMap<String, Boolean> feature_id_map = new HashMap<String, Boolean>(); 
            double sum = 0.0d;

            for (Text val: values) {
                String [] similarity_and_feature_id = val.toString().split(",");
                if (feature_id_map.containsKey(similarity_and_feature_id[1])) {
                    continue;
                }
                feature_id_map.put(similarity_and_feature_id[1], true);
                double similarity = Double.parseDouble(similarity_and_feature_id[0]); 
                sum += similarity;
            }
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (args.length < 4) {
            System.out.println("Usage: GraphConstructorMapReduceMinHashFeature <input path> <cluster path> <threshold> <cluster min size> [compress]");    
            System.exit(0);
        }
    ...
    public static Pair calculate_adl(String input_path) throws Exception {
        ...
        job_adl_calculator.setJarByClass(GraphConstructorMapReduceMinHashFeature.class);
    ...
    public static void calculate_tf(String input_path, double adl) throws Exception {
        ...
        job_tfidf_tf_calculator.setJarByClass(GraphConstructorMapReduceMinHashFeature.class);
    ...
    public static void calculate_idf(String input_path, int number_of_images) throws Exception {
        ...
        job_tfidf_idf_calculator.setJarByClass(GraphConstructorMapReduceMinHashFeature.class);
    ...
    public static void addminhashinfo(String input_path, String cluster_path, int min_cluster_size) throws Exception {
        ...
        job_addminhashinfo.setJarByClass(GraphConstructorMapReduceMinHashFeature.class);
    ...
    public static void construct_inverted_list(String input_path) throws Exception {
        ...
        job_inverted_list_constructor.setJarByClass(GraphConstructorMapReduceMinHashFeature.class);
    ...
    public static void compute_pairwise_similarity(String input_path, double threshold) throws Exception {
        ...
        job_pairwise_similarity_computation.setJarByClass(GraphConstructorMapReduceMinHashFeature.class);
